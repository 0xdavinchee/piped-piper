...
async function main() {
    ...
    let sv = await superValveFactory.deploy(process.env.HOST_ADDRESS, process.env.CFA_ADDRESS, fUSDCx, [
        vp1.address,
        vp2.address,
    ]);
    await sv.deployed();
    ...
    await hre.run("verify:verify", {
        address: fv1.address,
        constructorArguments: [fUSDC, "fUSDC Vault 1 Token", "vFUSDC1"],
    });
    await hre.run("verify:verify", {
        address: fv2.address,
        constructorArguments: [fUSDC, "fUSDC Vault 2 Token", "vFUSDC2"],
    });
    await hre.run("verify:verify", {
        address: vp1.address,
        constructorArguments: [fUSDCx, fv1.address],
    });
    await hre.run("verify:verify", {
        address: vp2.address,
        constructorArguments: [fUSDCx, fv2.address],
    });
    await hre.run("verify:verify", {
        address: sv.address,
        constructorArguments: [process.env.HOST_ADDRESS, process.env.CFA_ADDRESS, fUSDCx, [vp1.address, vp2.address]],
    });
}
